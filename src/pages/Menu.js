...
import { addProduct, getProducts, deleteProduct , addMenu , editMenu , deleteMenu, getMenu} from '../store/featureActions'
...
const Menu = () => {
    ...
    const getProduct = async () => {
        try {
            ...
            const response = await dispatch(getMenu(payload)).unwrap();
    ...
    const dltProduct = async (id) => {
        try {
            ...
            setIsDisabled(true)
            const response = await dispatch(deleteMenu(payload)).unwrap();
            setModalType("")
            await getProduct()
            setIsDisabled(false)
        }
    ...
    const editProduct = async (e, id) => {
        ...
        formData.append("menu_name", editproduct_name);
        ...
        try {
            if (!editproduct_name && !editdescription && !editprice) {
                ...
                setIsDisabled(true)
                await dispatch(editMenu(payload)).unwrap();
                setModalType("")
                await getProduct()
                resetForm()
                setIsDisabled(false)
    ...
    const populateForm = (product) => {
        ...
        setEditProduct_name(product?.menu_name)
                                        ...
                                        <div className="Product_modalDetail">
                                            ...
                                            <div className="Product_modalDetail_sect1 mb-3">
                                                <div className="ProdImg mb-3 prod">
                                                    <img src={`${process.env.REACT_APP_APIURL}${selectdata?.menuImage[0]}`} alt="" />
                                                </div>
                                                <p className="prod_name mb-2">Product: {selectdata?.menu_name}</p>
                                                <p className="price mb-2">Price: ${selectdata?.price}</p>

                                            </div>
                                            <p className="prodDesc">Description: {selectdata?.description}</p>
                                            <div className="modal_btnSect">
                                                {/* <button className="cta" onClick={() => viewModal(null, "editProduct")}>Edit</button> */}
                                                <button disabled={isDisabled} className="cta ctaDel" onClick={() => { dltProduct(selectdata?._id) }}>Delete</button>
                                            </div>
                                        </div>
                                    </Modal.Body>
                                </Modal>
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            <Modal.Body>
                                                ...
                                                <form onSubmit={(e) => editProduct(e, selectdata?._id)} action className="addProduct_ModalForm">
                                                    <div className="row">
                                                        <div className="col-12">
                                                            <div className="addProperty_image">
                                                                <div className="upload_box">
                                    ...
                                    return (<div key={index} className="col-12 col-md-6 col-lg-6 col-xl-6 col-xxl-4 mb-3">
                                        <div className="productCard">
                                            <div className="prodImg">
                                                <img src={`${process.env.REACT_APP_APIURL}${element?.menuImage[0]}`} alt="" />
                                            </div>
                                            <div className="prodDetail">
                                                <p className="prod_name">{element.menu_name}</p>
